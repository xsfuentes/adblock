...
## 'clean': delete sources and disable Adblock
...
clean() {
	...
	elog "$prefix/blocklist" deleted | rm -f "$prefix/blocklist"
 	elog "$prefix/blacklist" deleted | rm -f "$prefix/blacklist"
 	elog "$prefix/whitelist" deleted | rm -f "$prefix/whitelist"
	
	for i in "$listprefix"/lastmod-*; do
		elog $i deleted | rm -f $i
	done
	
	for i in "$listprefix"/source-*; do
		elog $i deleted | rm -f $i
	done
...
confgen() {
	...
	if [ -e "$prefix/whitelist" ]; then
		...
		sort -u "$listprefix"/blacklist | grep -v -f "$prefix/whitelist" > "$listprefix/blacklistc"
			[ -e "$prefix/blacklistc" ] && {
			cat "$prefix/blacklistc" >> "$listprefix/blocklist"
			rm -f "$prefix/blacklist"
			mv -f "$prefix/blacklistc" "$prefix/blacklist"
			}
	else
		sort -u "$listprefix"/source-* > "$listprefix/blocklist"
		sort -u "$listprefix"/blacklist > "$listprefix/blocklist"
	fi
